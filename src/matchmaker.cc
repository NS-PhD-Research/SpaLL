...
bool sortByMetricDist(const std::pair<int,int> &p1,
              const std::pair<int,int> &p2){
    return (p1.second < p2.second);
}

bool sortByMetricLat(const std::pair<int,float> &p1,
...
int MatchMaker::selectHostForProducer(int prod_id, bool replica, int cons_id){
    ...
    if(ginfo_->ptype == PlacementType::DIST){
        ...
        std::sort(dist_hosts.begin(), dist_hosts.end(), sortByMetricDist);
            ...
            if(host->producer_load_ + 1 <= host->producer_load_threshold_){
                ...
                if(host->remaining_storage_capacity_ >= producer->total_transfer_size_){
                    ...
                    if(replica){
                        ...
                        float rep_overhead = l1 + l3 + l4 + l5 + l6 + l7;
                        // std::cout << rep_overhead << std::endl;

                        ginfo_->log_mutex.lock();
                        ginfo_->fout << "RO " << rep_overhead << std::endl;
                        ginfo_->log_mutex.unlock();
                    } else {
                        ...
                        // std::cout << host_selection_latency << std::endl;
        ...
        producer->dls_mutex_.unlock();
        std::sort(latency_hosts.begin(), latency_hosts.end(), sortByMetricLat);
