            ...
            switch(entity){
                ...
                case 1:
                    ...
                    for(auto host: ginfo.hosts){
                        ...
                        // score = GenerateRandomFloat(0.0, 1.0);
                        float latency;
                        if(score > 0.66)
    ...
    for(auto sub: sub_hosts){
        ...
        sub_thds.push_back(std::thread([id, prod, host](){
            ...
            for(int i = 0; i < NCHUNKS; i++){
                ...
                ginfo.log_mutex.lock();
                ginfo.fout << "E2EL " << ginfo.consumers[id]->latency_hosts_map_[host] + ginfo.producers[prod]->latency_hosts_map_[host] + 2 * GenerateRandomFloat(-1.0, 1.0) << std::endl;
                ginfo.log_mutex.unlock();
            }
        }));
    }
